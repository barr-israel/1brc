...
pub struct MyHashMap {
    data: Box<[(StationName, StationEntry); SIZE]>,
}
...
impl MyHashMap {
    pub fn new() -> MyHashMap {
        let mut data;
        unsafe {
            data = Box::<[(StationName, StationEntry); SIZE]>::new_uninit().assume_init();
        }
    ...
    pub fn insert_measurement(&mut self, name: StationName, measurement: i32) {
        ...
        let entry = unsafe {
            loop {
                let index = hash & MASK;
                let potential_location = self.data.get_unchecked_mut(index & MASK);
                if potential_location.0.ptr.is_null() {
                    potential_location.0 = name;
                    break &mut potential_location.1;
                }
                if potential_location.0 == name {
                    break &mut potential_location.1;
                }
    ...
    pub fn iter(&self) -> impl Iterator<Item = &(StationName, StationEntry)> {
        self.data.iter().filter(|(name, _)| !name.ptr.is_null())
    }
}
